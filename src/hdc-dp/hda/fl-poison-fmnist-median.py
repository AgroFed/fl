...
from libs import agg, data, fl, hdc, helper, log, ltbl, nn, plot, poison, resnet, sim, wandb
...
fedargs.agg_rule = agg.Rule.Median
    ...
    if epoch > 0:
        ...
        if "attack" in global_test_output:
            if "attack_success_rate" in global_test_output["attack"]:
                wb.log({"epoch": epoch, "time": time.time(), "attack_success_rate": global_test_output["attack"]["attack_success_rate"]})
            ...
            if "misclassification_rate" in global_test_output["attack"]:
                wb.log({"epoch": epoch, "time": time.time(), "misclassification_rate": global_test_output["attack"]["misclassification_rate"]})
        ...
        if backdoor_attack["is"]:
            ...
            wb.log({"epoch": epoch, "time": time.time(), "backdoor_success_rate": backdoor_test_output["accuracy"]})
            log.jsoninfo(backdoor_test_output, "Backdoor Test Outut after Epoch {} of {}".format(epoch, fedargs.epochs))

        # CSV logging
        ltbl.ar({'epoch': epoch,'acc': global_test_output["accuracy"], 'test_loss': global_test_output["test_loss"], 'asr': global_test_output["attack"]["attack_success_rate"], 'msr': global_test_output["attack"]["misclassification_rate"]})
