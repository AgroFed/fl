...
class FedArgs():
    def __init__(self):
        ...
        self.tb = SummaryWriter('../../out/runs/federated/FLTrust/mn(12)-sine-100-dot-3-nu', comment="Centralized Federated training")
...
mal_clients = [c for c in range(12)]
...
cosine_attack = True
cosargs = {"poison_percent": 1, "scale_dot": 3, "scale_norm": 100}
...
for _epoch in tqdm(range(fedargs.epochs)):
    ...
    if epoch > 1:
        ...
        client_models = {client: copy.deepcopy(global_model) for client in clients if client not in mal_clients}
        for client in clients:
            if client in mal_clients:
                client_models[client] = copy.deepcopy(init_model)
