...
'''
def set_hdc_labels(flip_labels = None):
    if flip_labels is None:
        hdc_dp_attack["labels"] = {4: 6} if hdc_dp_attack["is"] and hdc_dp_attack["func"] is poison.hdc_dp else None
        hdc_dp_attack["labels"] = {label: fedargs.labels[(index + 1) % len(fedargs.labels)] for index, label in enumerate(fedargs.labels)} if hdc_dp_attack["is"] and hdc_dp_attack["func"] is poison.hdc_dp_attack else hdc_dp_attack["labels"]
    else:
        hdc_dp_attack["labels"] = flip_labels
'''
